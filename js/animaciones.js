function animaciones() {
    let pantalla = window.innerWidth; window.addEventListener('scroll', () => {
        ...
        let pos = skill.getBoundingClientRect().top; let pos2 = skill2.getBoundingClientRect().top; let pos3 = design.getBoundingClientRect().top; if (pantalla > 750) {
            if (pos <= 600) {
                for (i = 0; i < barras.length; i++) { barras[i].style.animation = 'barrasColores 500ms alternate forwards' }
            ...
            if (pos > 600) {
                for (i = 0; i < barras.length; i++) { barras[i].style.animation = 'barrasSinColores 500ms alternate forwards' }
        ...
        if (pantalla < 750) {
            if (pos <= 600) {
                for (i = 0; i < barras.length; i++) { barras[i].style.animation = 'barrasColores 800ms alternate forwards' }
                for (i = 0; i < secciones.length; i++) { secciones[i].style.animation = 'deslizarSkillMostrar 1s alternate forwards' }
            }
            if (pos3 > 600) { design.style.animation = 'designClipShow 2s alternate forwards' }
            if (pos > 600) {
